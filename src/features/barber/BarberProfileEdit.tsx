...
//
// Held apart from the markup so the screen below reads as the list of sections
// the board draws, and so a change to what is loaded cannot disturb layout.
// ---------------------------------------------------------------------------

function useProfileEditState(user: AuthUser) {
  ...
  return {
    form, setForm, update, saving, setSaving, loading, setLoading,
    avatarUrl, setAvatarUrl, avatarUploading, setAvatarUploading, avatarError, setAvatarError,
    photos, setPhotos, photosUploading, setPhotosUploading, photosError, setPhotosError,
    deletingPhotoId, setDeletingPhotoId, portfolioInputRef,
    serviceMode, setServiceMode, bookingMode, setBookingMode, bufferMinutes, setBufferMinutes,
    isOnline, setIsOnline, scheduleSummary, setScheduleSummary,
    services, setServices, barberId, setBarberId, isVerified, setIsVerified,
    ratingAvg, setRatingAvg, ratingCount, setRatingCount, jobsCompleted, setJobsCompleted,
  };
}

type State = ReturnType<typeof useProfileEditState>;

function useProfileEditLoad(s: State, user: AuthUser) {
  useEffect(() => {
    if (!user?.id) {
      s.setLoading(false);
      return;
    ...
    (async () => {
      try {
        ...
        if (profileRes.ok) {
          ...
          if (cancelled) return;
          s.setBarberId(p.id);
          s.setIsVerified(!!p.is_verified);
          s.setRatingAvg(p.rating_avg != null ? Number(p.rating_avg) : null);
          s.setRatingCount(Number(p.rating_count || 0));
          s.setForm({
          ...
          if (p.service_mode === 'mobile') s.setServiceMode('come_to_customer');
          else if (p.service_mode === 'both') s.setServiceMode('both');
          else if (p.service_mode === 'in_shop') s.setServiceMode('in_shop');
          if (p.booking_mode === 'instant' || p.booking_mode === 'request_only') s.setBookingMode(p.booking_mode);
          if (p.buffer_minutes != null) s.setBufferMinutes(String(Number(p.buffer_minutes)));
          s.setIsOnline(!!p.is_online);

          if (p.id) {
            const servicesRes = await authFetch('/api/services');
            if (servicesRes.ok) {
              const all: Service[] = await servicesRes.json();
              if (!cancelled) s.setServices(all.filter((x) => x.barber_profiles?.id === p.id));
            }
          }
        }
        ...
        if (scheduleRes.ok) {
          ...
          if (!cancelled && Array.isArray(rows) && rows.length > 0) {
            ...
            if (available.length === 0) {
              s.setScheduleSummary('No working hours set');
            } else {
              const days = available.map((r: any) => DAY_NAMES[r.day_of_week]).join(', ');
              const sample = available[0];
              s.setScheduleSummary(`${days}  • ${sample.start_time?.slice(0, 5)} – ${sample.end_time?.slice(0, 5)}`);
            }
          } else if (!cancelled) {
            s.setScheduleSummary('Not set yet');
          }
        }
        ...
        if (bookingsRes.ok) {
          ...
          if (!cancelled && Array.isArray(rows)) {
            s.setJobsCompleted(rows.filter((b: any) => b.status === 'completed').length);
          }
        }
      } catch (err) {
...
/** The three picture actions: the avatar, adding to the portfolio and
 *  removing from it. */
function usePhotoActions(s: State, user: AuthUser, setUser: (u: AuthUser) => void) {
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    e.target.value = '';
    if (!file) return;
    s.setAvatarError(null);
    s.setAvatarUploading(true);
    try {
  ...
  async function handlePortfolioAdd(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (selected.length > 6) {
      s.setPhotosError('You can upload at most 6 photos at a time.');
      return;
    }
    s.setPhotosError(null);
    s.setPhotosUploading(true);
    try {
      ...
      s.setPhotosUploading(false);
    }
  }

  async function handlePortfolioDelete(id: string) {
    s.setPhotosError(null);
    s.setDeletingPhotoId(id);
    try {
  ...
  return { handlePhotoChange, handlePortfolioAdd, handlePortfolioDelete };
}

function useSaveProfile(s: State) {
  return async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    if (!s.form.display_name.trim() || !s.form.address_text.trim() || !s.form.bio.trim()) {
    ...
    try {
      const response = await authFetch('/api/barber/profile', {
        method: 'PUT',
        body: JSON.stringify({
          display_name: s.form.display_name || undefined,
          shop_name: s.form.shop_name || undefined,
          bio: s.form.bio || undefined,
          address_text: s.form.address_text || undefined,
          experience_years: s.form.experience_years ? Number(s.form.experience_years) : undefined,
          phone: normalisePhone(s.form.phone) || undefined,
          service_mode: s.serviceMode === 'in_shop' ? 'in_shop' : s.serviceMode === 'both' ? 'both' : 'mobile',
          booking_mode: s.bookingMode,
          buffer_minutes: Number(s.bufferMinutes),
...
//
// The board draws this page as a stack of titled sections. Each one is a
// component here so a correction to a section is a change in one place and
// the order of the page stays readable at a glance in the screen below.
//
// NOTE: the header is deliberately NOT the shared ScreenHeader — the board
// draws this one with a smaller arrow and no centring wrapper, and the design
// is contractually locked, so it stays as its own piece until the design says
// otherwise.
// ---------------------------------------------------------------------------
...
function AvatarBlock({ s, user, onPickPhoto }: { s: State; user: AuthUser; onPickPhoto: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
  return (
    <div className="flex flex-col items-center px-5 pt-2">
      <div className="relative">
        <div className="w-[100px] h-[100px] rounded-[16px] bg-[#f2f1fa] flex items-center justify-center overflow-hidden">
          {s.avatarUrl || user?.avatar_url ? (
            <img src={s.avatarUrl || user?.avatar_url} className="w-full h-full object-cover" alt="" />
          ) : (
        ...
        <label
          className={`absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#1c1b1f] flex items-center justify-center ${
            s.avatarUploading ? 'opacity-60 cursor-default' : 'cursor-pointer'
          }`}
        >
          ...
          <input
            ...
            className="hidden"
            disabled={s.avatarUploading}
            onChange={onPickPhoto}
          />
        </label>
      </div>
      <h1 className="text-[22px] font-bold text-[#1c1b1f] mt-4">
        {s.form.display_name || user?.full_name || 'Your Name'}
      </h1>
      <p className="text-[14px] font-semibold text-[#1c1b1f] mt-0.5">
        {s.avatarUploading ? 'Uploading…' : 'Change Photo'}
      </p>
      {s.avatarError && (
        <p className="text-[12px] font-medium text-red-600 mt-1 text-center">{s.avatarError}</p>
      )}
...
function BasicInformationSection({ s }: { s: State }) {
  return (
    <section>
      ...
      <div className="flex flex-col gap-3">
        <PillInput
          ...
          onChange={(e) => s.update('display_name', e.target.value)}
        />
        <PillInput
          icon={<MapPin className="w-5 h-5" strokeWidth={1.5} />}
          placeholder="City / Area (required)"
          value={s.form.address_text}
          onChange={(e) => s.update('address_text', e.target.value)}
        />
        <div className="flex items-start gap-3 w-full rounded-[24px] border-[0.75px] border-[#d2dbe9] bg-white px-5 py-[15px] min-h-[120px]">
          ...
          <textarea
            ...
            value={s.form.bio}
            onChange={(e) => s.update('bio', e.target.value)}
            rows={3}
...
function ContactLocationSection({ s }: { s: State }) {
  return (
    <section>
      ...
      <div className="flex flex-col gap-3">
        ...
        <PillInput
          ...
          value={s.form.shop_name}
          onChange={(e) => s.update('shop_name', e.target.value)}
        />
        <button
          type="button"
...
function ServiceModeSection({ s }: { s: State }) {
  return (
    <section>
      ...
      <div className="flex gap-3">
        ...
        <button
          type="button"
          onClick={() => s.setServiceMode('come_to_customer')}
          className={`flex-1 py-[15px] rounded-full text-[14px] font-semibold transition-colors ${
            s.serviceMode === 'come_to_customer'
              ? 'bg-[#1c1b1f] text-white'
              : 'bg-white text-[#6c6a75] border-[0.75px] border-[#d2dbe9]'
          }`}
        >
...
function AvailabilitySection({ s }: { s: State }) {
  return (
    <section>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                s.setIsOnline(!!body.is_online);
              } catch (err) {
                s.setIsOnline(!next);
                toast.error(`${err instanceof Error ? err.message : err}`);
              }
            }}
            aria-label={s.isOnline ? 'Go offline' : 'Go online'}
            className={`w-11 h-6 rounded-full transition-colors relative ${s.isOnline ? 'bg-[#1c1b1f]' : 'bg-[#d8d6e0]'}`}
          >
            <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-transform ${s.isOnline ? 'translate-x-5' : 'translate-x-0.5'}`} />
          </button>
        </div>
...
function BookingPreferencesSection({ s }: { s: State }) {
  return (
    <section>
      ...
      <div className="flex flex-col gap-3">
        {([
          ...
          <button
            key={opt.value}
            type="button"
            onClick={() => s.setBookingMode(opt.value)}
          ...
          >
            <span
              className={`w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center ${
                s.bookingMode === opt.value ? 'border-[#1c1b1f]' : 'border-[#c9c6d4]'
              }`}
            >
              {s.bookingMode === opt.value && <span className="w-2.5 h-2.5 rounded-full bg-[#1c1b1f]" />}
            </span>
            <span>
      ...
      <div className="relative">
        <select
          ...
          onChange={(e) => s.setBufferMinutes(e.target.value)}
    ...
    <div className="rounded-[16px] border-[0.75px] border-[#d2dbe9] bg-white p-4">
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        {s.services.length === 0 ? (
          ...
          s.services.map((x) => (
            <span key={x.id} className="px-3 py-1.5 rounded-full bg-[#f8f8f8] text-[12px] font-semibold text-[#1c1b1f]">
              {x.name}
            </span>
          ))
        )}
  ...
  return (
    <div className="rounded-[16px] border-[0.75px] border-[#d2dbe9] bg-white p-4">
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          disabled={s.photosUploading}
          onClick={() => s.portfolioInputRef.current?.click()}
          className="text-[12px] font-semibold text-[#1c1b1f] disabled:opacity-60"
        >
          {s.photosUploading ? '• Uploading…' : '• Add Photo'}
        </button>
      </div>
      <div className="flex flex-wrap gap-2 mt-3">
        {s.photos.map((photo) => (
          <div key={photo.id} className="relative w-[56px] h-[56px]">
            ...
            <button
              ...
              disabled={s.deletingPhotoId === photo.id}
        ...
        {Array.from({ length: Math.max(3 - s.photos.length, 0) }).map((_, i) => (
      ...
      {s.photosError && (
        <p className="text-[12px] font-medium text-red-600 mt-2">{s.photosError}</p>
      )}
      <p className="text-[12px] font-medium text-[#a09cab] mt-3">
...
function VerificationSection({ s, navigate }: { s: State; navigate: (to: string) => void }) {
  return (
    <section>
      ...
      <div className="rounded-[16px] bg-[#f8f8f8] p-4 flex items-center justify-between">
        <div>
          <p className="text-[14px] font-bold text-[#1c1b1f]">{s.isVerified ? 'Approved' : 'Pending'}</p>
          <p className="text-[12px] font-medium text-[#a09cab] mt-0.5">
            {s.isVerified ? 'Verified badge visible on profile' : 'Verification under review'}
          </p>
        </div>
        <button
          type="button"
...
function PerformanceSection({ s, navigate }: { s: State; navigate: (to: string) => void }) {
  return (
    <section>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="rounded-[16px] border-[0.75px] border-[#d2dbe9] bg-white py-4 flex flex-col items-center gap-1">
          ...
          <p className="text-[13px] font-bold text-[#1c1b1f] mt-1">{s.jobsCompleted}</p>
          <p className="text-[13px] font-semibold text-[#1c1b1f]">Jobs</p>
        </div>
        <div className="rounded-[16px] border-[0.75px] border-[#d2dbe9] bg-white py-4 flex flex-col items-center gap-1">
          <Clock className="w-5 h-5 text-[#1c1b1f]" strokeWidth={1.75} />
          <p className="text-[13px] font-bold text-[#1c1b1f] mt-1">{s.ratingCount}</p>
...
function SafetySupportSection({ navigate }: { navigate: (to: string) => void }) {
  return (
    <section>
...
export default function BarberProfileEdit() {
  ...
  const { handlePhotoChange, handlePortfolioAdd, handlePortfolioDelete } = usePhotoActions(s, user, setUser);
  const handleSave = useSaveProfile(s);
  ...
  return (
    <div className="min-h-screen bg-white pb-[88px]">
      ...
      {/* Avatar block */}
      <AvatarBlock s={s} user={user} onPickPhoto={handlePhotoChange} />

      <form onSubmit={handleSave} className="px-5 mt-8 flex flex-col gap-8">
        {/* Basic Information */}
        <BasicInformationSection s={s} />

        {/* Contact & Location */}
        <ContactLocationSection s={s} />

        {/* Service Mode */}
        <ServiceModeSection s={s} />

        {/* Availability */}
        <AvailabilitySection s={s} />

        {/* Booking Preferences */}
        <BookingPreferencesSection s={s} />

        {/* Services */}
        <ServicesCard s={s} navigate={navigate} />

        {/* Portfolio */}
        <PortfolioCard s={s} onAdd={handlePortfolioAdd} onDelete={handlePortfolioDelete} />

        {/* Verification */}
        <VerificationSection s={s} navigate={navigate} />

        {/* Performance */}
        <PerformanceSection s={s} navigate={navigate} />

        {/* Safety & Support */}
        <SafetySupportSection navigate={navigate} />
