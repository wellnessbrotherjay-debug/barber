...
// The screen is one long column of independent blocks. Each block below is its
// own piece so that the loading logic at the top of the screen is readable and
// so a change to one block cannot disturb its neighbours. The markup and the
// measurements are the Figma board's and are unchanged.

function CustomerProfileBlock({ job }: { job: Job }) {
  return (
...
function CustomerNotesBlock({ notes }: { notes: Job['notes'] }) {
  return (
    /* Notes From Customer */
...
function LocationBlock({ job, address }: { job: Job; address: string }) {
  return (
    /* Location */
    <div className="pt-8">
...
function BookingInfoBlock({ job }: { job: Job }) {
  return (
    <>
      {/* Booking Info */}
      <div className="px-5 pt-8">
...
/**
 * Which buttons a job shows depends entirely on its status, so that decision
 * lives here on its own rather than as three nested conditionals in the middle
 * of the screen.
 */
function JobActions({
  job,
...
export default function JobDetail() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col pb-8">
      ...
      {job && (
        <>
          <CustomerProfileBlock job={job} />

          <CustomerNotesBlock notes={job.notes} />

          <LocationBlock job={job} address={address} />

          <BookingInfoBlock job={job} />
          ...
          <JobActions
            ...
            isPast={isPast}
            bookingDateInPast={bookingDateInPast}
            navigate={navigate}
          />
        </>
      )}
